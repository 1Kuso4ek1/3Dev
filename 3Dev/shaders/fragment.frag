...
vec3 FresnelShlick(float cosTh, vec3 f0)
{
...
vec3 CalcLight(Light light, vec3 norm)
{
    ...
    lo += (kdif * diffuse / pi + spc) * rad * ndotl;
...
void main()
{
    ...
    vec3 diffuse = (ndiff ? texture(diff, coord).xyz : vec3(1.0));
    vec3 amb = (vec3(0.03) * diffuse * (nao ? texture(ao, coord).xyz : vec3(1.0))) + total;

    color = vec4(emission + (reflection * amb) + amb, (nopacity ? texture(opacity, coord).x : 1.0));
}
