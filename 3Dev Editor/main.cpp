...
int main(int argc, char* argv[]) {
	...
	engine.EventLoop([&](sf::Event& event) {
		if (gui.CatchEvent(event, engine.GetWindow()) == modelButton.ID && !animationdialog && !animationeditdialog && !lightdialog && !modeleditdialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog) modeldialog = !modeldialog;
		if (gui.CatchEvent(event, engine.GetWindow()) == animationButton.ID && !modeldialog && !lightdialog && !animationeditdialog && !modeleditdialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog) animationdialog = !animationdialog;
		if (gui.CatchEvent(event, engine.GetWindow()) == lightButton.ID && !modeldialog && !animationdialog && !animationeditdialog && !modeleditdialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog) lightdialog = !lightdialog;
		if (gui.CatchEvent(event, engine.GetWindow()) == editButton.ID && !animationdialog && !animationeditdialog && !lightdialog && !modeldialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog && selected != -1) 
		{
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == editButton1.ID && !modeleditdialog && !animationdialog && !lightdialog && !modeldialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog && selectedAnim != -1) 
		{
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == editButton2.ID && !animationdialog && !animationeditdialog && !lightdialog && !modeldialog && !lighteditdialog && !savedialog && !loaddialog && !exportdialog && !modeleditdialog && selectedLight != -1) 
		{
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == saveButton.ID && !modeldialog && !animationdialog && !animationeditdialog && !modeleditdialog && !lighteditdialog && !loaddialog && !exportdialog) savedialog = !savedialog;
		if (gui.CatchEvent(event, engine.GetWindow()) == loadButton.ID && !modeldialog && !animationdialog && !animationeditdialog && !modeleditdialog && !lighteditdialog && !savedialog && !exportdialog) loaddialog = !loaddialog;
		if (gui.CatchEvent(event, engine.GetWindow()) == exportButton.ID && !modeldialog && !animationdialog && !animationeditdialog && !modeleditdialog && !lighteditdialog && !savedialog && !loaddialog) exportdialog = !exportdialog;
		
		if(modeldialog)
		{
			if (modelDialog.CatchEvent(event, engine.GetWindow()) == ok.ID)
			{
		...
		if(animationdialog)
		{
			if (animationDialog.CatchEvent(event, engine.GetWindow()) == ok.ID)
			{
		...
		if(lightdialog)
		{
			if (lightDialog.CatchEvent(event, engine.GetWindow()) == ok.ID)
			{
		...
		if(modeleditdialog)
		{
			int temp = modelEditDialog.CatchEvent(event, engine.GetWindow());
			if (temp == ok.ID)
			{
		...
		{
			int temp = animationEditDialog.CatchEvent(event, engine.GetWindow());
			if (temp == ok.ID)
			{
		...
		if(lighteditdialog)
		{
			int temp = lightEditDialog.CatchEvent(event, engine.GetWindow());
			if (temp == ok.ID)
			{
		...
		if(savedialog)
		{
			if (saveLoadDialog.CatchEvent(event, engine.GetWindow()) == ok.ID)
			{
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == plusButton.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == minusButton.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == plusButton1.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == minusButton1.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == plusButton2.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == minusButton2.ID) {
		...
		if (gui.CatchEvent(event, engine.GetWindow()) == infoButton.ID) {
			modelmove = true;
			animmove = lightmove = false;
		}
		else if (gui.CatchEvent(event, engine.GetWindow()) == infoButton2.ID) {
			modelmove = lightmove = false;
			animmove = true;
		}
		else if (gui.CatchEvent(event, engine.GetWindow()) == infoButton4.ID) {
	...
	engine.Loop([&]() {
		...
		if (right) {
			if(showcursor) engine.GetWindow().setMouseCursorVisible(!showcursor);
			showcursor = false;
			cam.Mouse(engine.GetWindow());
		}
		else if (!showcursor) {
			engine.GetWindow().setMouseCursorVisible(!showcursor);
			showcursor = true;
		}
		...
		if(modeldialog) modelDialog.Draw(engine.GetWindow());		
		if(animationdialog) animationDialog.Draw(engine.GetWindow());
		if(lightdialog) lightDialog.Draw(engine.GetWindow());
		if(modeleditdialog) modelEditDialog.Draw(engine.GetWindow());		
		if(animationeditdialog) animationEditDialog.Draw(engine.GetWindow());		
		if(lighteditdialog) lightEditDialog.Draw(engine.GetWindow());
		if(savedialog || loaddialog || exportdialog) saveLoadDialog.Draw(engine.GetWindow());
		
		gui.Draw(engine.GetWindow());
	});
